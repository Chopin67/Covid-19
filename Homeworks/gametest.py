...
def tick(keys):
   ...
   if p1healthbarforeground.width <0:
       ...
       camera.draw(win)
       winplayer = p2
       scoree = gamebox.from_text(camera.x, camera.y + 200, "Score:", "Arial", 50, "white", bold=True)
       sc = gamebox.from_text(camera.x + 100, camera.y + 200, str(ticks // 30), "Arial", 50, "white", bold=True)
       camera.draw(scoree)
       camera.draw(sc)
       gamebox.pause()
